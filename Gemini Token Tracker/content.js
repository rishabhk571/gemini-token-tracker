...
(() => {
  ...
  function hasConfirmedUploadMarker(chip) {
    ...
    try {
      if (chip.querySelector('[data-test-id="cancel-button"], [aria-label^="Remove file "]')) {
        return true;
      }
    } catch (_) {}
  ...
  function shouldUseDomAttachmentFallback() {
    return !!(currentChatId || totalFileTokens > 0 || networkFallbackTokens > 0 || attachmentInteractionSeen);
  }
  ...
  function isConfirmedUploadChip(chip, userMessageElement) {
    ...
    if (hasConfirmedUploadMarker(chip)) return true;
    if (!shouldUseDomAttachmentFallback()) return false;

    return looksLikeFileDescriptor(getAttachmentChipText(chip));
  }
  ...
  async function recalculate() {
    ...
    if (!zeroState && trackedFileCount === 0 && domChipTokens > 0 && currentChatId) {
    ...
    var finalInputTokens = counts.input + attachTokens + canvasTokens;
  ...
  function requestCanvasMonacoText() {
  ...
  function getCanvasRoot() {
    var root = null;
    try { root = deepQuery('code-immersive-panel', document); } catch (_) {}
    if (root) return root;
    try {
      var editor = deepQuery('[data-test-id="code-editor"]', document);
      if (editor) return editor.closest('code-immersive-panel') || editor;
    } catch (_) {}
    try {
      var content = deepQuery('[data-test-id="canvas-content"]', document);
      if (content) return content.closest('code-immersive-panel') || content;
    } catch (_) {}
    return null;
  }
  ...
  function getCanvasEditorText(root) {
    ...
    var editor = null;
    try { editor = deepQuery('[data-test-id="code-editor"]', root); } catch (_) {}
    if (!editor) {
      try { editor = deepQuery('xap-code-editor', root); } catch (_) {}
    }
    if (editor) {
      try {
        var lines = deepQuery('.monaco-editor .view-lines', editor);
        var raw = lines ? (lines.innerText || lines.textContent || '') : '';
        if (raw) {
    ...
    try {
      var content = deepQuery('[data-test-id="canvas-content"]', root);
      if (content) {
        canvasTextSource = 'dom-content';
  ...
  function scheduleCanvasScan(root, immediate) {
    ...
    canvasDebounceTimer = setTimeout(function() {
      ...
      var isCode = false;
      try { isCode = !!deepQuery('[data-test-id="code-editor"]', target); } catch (_) {}
      ...
      var tokenText = (isCode && canvasCountMode === 'code') ? ('```\n' + text + '\n```') : text;
      try {
