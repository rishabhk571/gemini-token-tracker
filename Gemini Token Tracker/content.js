...
(() => {
  ...
  var canvasTokens = 0;
  var canvasRoot = null;
  var canvasRootChatId = '';
  var canvasRootRouteKey = '';
  ...
  var canvasMonacoChatId = '';
  var canvasMonacoRouteKey = '';
  var canvasBridgeInjected = false;
  var canvasLastRequestAt = 0;
  var canvasPendingRequestId = '';
  var canvasScanSeq = 0;
  var staleCanvasRoots = typeof WeakSet !== 'undefined' ? new WeakSet() : null;
  var lastCanvasRouteKey = '';
  ...
  function buildMeterEstimateIndicator(breakdown) {
    ...
    if (sources.chosenAttachmentSource === 'history-storage' &&
      ...
      return {
        ...
        title: 'A previously sent file (' + sources.chosenAttachmentTokens.toLocaleString() + ' tk) is persisting in this conversation\'s context window on every turn. Start a new chat when you want to remove it from Gemini\'s active context.',
      };
    }
  ...
  function clearHistoryAttachmentTokens() {
    // This action is only for stale cache ghosts. If the current chat still
    // exposes any real attachment evidence, keep the count intact.
    if (hasActiveAttachmentEvidenceForCurrentChat()) {
      scheduleRecalc();
      return;
    }
  ...
  function hasActiveAttachmentEvidenceForCurrentChat() {
    try {
      if (getTrackedFileCount() > 0) return true;
      if (totalFileTokens > 0 || networkFallbackTokens > 0) return true;
      if (hasConfirmedUploadChipAnywhere()) return true;

      if (!isZeroStateEmptyPage()) {
        var messages = scrapeConversation();
        for (var i = 0; i < messages.length; i++) {
          if (messages[i].role !== 'user') continue;
          if ((messages[i].docTokens || 0) > 0) return true;
          if ((messages[i].userImageTokens || 0) > 0) return true;
        }
      }
    } catch (_) {}
  ...
  var localTabTier = '';
  var localTabTierScope = '';
  var currentPlanScope = '';
  var PLAN_RETRY_DELAYS_MS = [0, 350, 1200, 2600, 5200];
  var PLAN_WEAK_REPROBE_MIN_MS = 5000;
  var PLAN_WEAK_REPROBE_DELAY_MS = 2200;
  var PLAN_CACHE_TTL_PAID_MS = 30 * 24 * 60 * 60 * 1000;
  var PLAN_CACHE_TTL_FREE_MS = 6 * 60 * 60 * 1000;
  var lastWeakPlanProbeAt = 0;

  function getPlanScopeFromHref(href) {
    var raw = String(href || '');
    var match = raw.match(/\/u\/([^\/?#]+)/i);
    if (match && match[1]) return String(match[1]).toLowerCase();
    return 'default';
  }

  function getScopedPlanCacheKey(scope) {
    return 'gtm_cached_tier_' + String(scope || 'default');
  }

  function syncPlanScope(href) {
    var scope = getPlanScopeFromHref(href || window.location.href);
    if (scope !== currentPlanScope) {
      currentPlanScope = scope;
      // Prevent account A from reusing account B's in-memory tier.
      if (localTabTierScope !== scope) {
        localTabTier = '';
        localTabTierScope = '';
      }
    }
  ...
  function decodeScopedTierCacheEntry(rawValue) {
    var fallbackTier = normalizePlanTier(rawValue);
    if (fallbackTier) {
      return { tier: fallbackTier, ts: 0, source: 'legacy' };
    }
    ...
    var parsedTier = normalizePlanTier(parsed.tier || parsed.plan || '');
    if (!parsedTier) {
      return { tier: '', ts: 0, source: '' };
    }

    var parsedTs = Number(parsed.ts || parsed.at || 0);
    return {
      tier: parsedTier,
      ts: (Number.isFinite(parsedTs) && parsedTs > 0) ? Math.floor(parsedTs) : 0,
      source: String(parsed.source || ''),
    };
  }

  function isScopedTierCacheUsable(entry) {
    if (!entry || !entry.tier) return false;
    if (!entry.ts) return entry.source !== 'legacy'; // ignore legacy unscoped/untimed cache

    var ageMs = Date.now() - entry.ts;
    if (!Number.isFinite(ageMs) || ageMs < 0) return false;

    var ttlMs = entry.tier === 'Free' ? PLAN_CACHE_TTL_FREE_MS : PLAN_CACHE_TTL_PAID_MS;
    return ageMs <= ttlMs;
  }

  function encodeScopedTierCacheEntry(tier, source) {
  ...
  function setPlanDetectionState(tier, confidence, source, signalText, planScope) {
    var scope = String(planScope || currentPlanScope || syncPlanScope(window.location.href) || 'default');
    var normalizedTier = normalizePlanTier(tier);
    ...
    currentPlanTierSignal = normalizePlanSignalText(signalText);
    if (normalizedTier) {
      localTabTier = normalizedTier;
      localTabTierScope = scope;
    }
  }
  ...
  function getUpgradeOfferTextFallback() {
    var selectors = [
    ...
    for (var i = 0; i < selectors.length; i++) {
      var sel = selectors[i];
      try {
        if (typeof deepQueryAll === 'function') {
          nodes = nodes.concat(deepQueryAll(sel, document));
        } else {
    ...
    if (typeof uniqueElements === 'function') {
      nodes = uniqueElements(nodes);
    }
    ...
    for (var j = 0; j < nodes.length; j++) {
      ...
      try {
        ...
        if (typeof el.getAttribute === 'function') {
          attrs.push(el.getAttribute('aria-label') || '');
          attrs.push(el.getAttribute('title') || '');
          attrs.push(el.getAttribute('data-tooltip') || '');
          attrs.push(el.getAttribute('data-test-id') || '');
      ...
      var hasOfferVerb = /\b(upgrade|get|try|unlock|switch)\b/.test(text);
      var mentionsPlanWord = /\b(plus|premium|advanced|pro|ultra)\b/.test(text);
      if (!hasOfferVerb && !mentionsPlanWord) continue;

      if (text.includes('google ai plus') || text.includes('ai plus')) {
        return text;
      }
      if (text.includes('google ai pro') || text.includes('ai pro') ||
          text.includes('google ai ultra') || text.includes('ai ultra')) {
        return text;
      }
      if (!genericUpgrade && (text.includes('upgrade') || (hasOfferVerb && mentionsPlanWord))) {
        genericUpgrade = text;
      }
    }
  ...
  function normalizePlanSignalText(text) {
    var compact = String(text || '').replace(/\s+/g, ' ').trim();
    if (!compact) return '';
    var maxLen = 120;
    if (compact.length > maxLen) {
      compact = compact.slice(0, maxLen - 3).trim() + '...';
    }
    return compact;
  }
  ...
  function tierFromUpsellOfferText(text) {
    var t = String(text || '').toLowerCase();
    if (!t) return '';

    var hasOfferVerb = /\b(upgrade|get|try|unlock|switch)\b/.test(t);
    if (!hasOfferVerb) return '';

    // Free accounts often surface CTAs inviting an upgrade to Plus-tier naming.
    // Keep this constrained so it cannot accidentally mark paid plans as Free.
    var offersPlus =
      t.includes('google ai plus') ||
      t.includes('ai plus') ||
      /\bplus\b/.test(t) ||
      /\bpremium\b/.test(t) ||
      /\badvanced\b/.test(t);
    var offersHigherPaid =
    ...
    if (offersPlus && !offersHigherPaid) return 'Free';
  ...
  function shouldPersistPlanTier(tier, confidence, source) {
    if (!isKnownPlanTier(tier)) return false;
    if (confidence === 'high') return true;

    // Pillbox is a stable on-page plan signal; persist medium confidence from it
    // so refreshes can recover even if profile text is temporarily unavailable.
    if (confidence === 'medium' && source === 'pillbox') return true;
    if (confidence === 'medium' && source === 'upsell-offer' && tier === 'Free') return true;
    return false;
  }

  function shouldForceFreeFallback(tier, source, cachedTier) {
    if (isPaidPlanTier(cachedTier)) return false;
    if (tier !== 'Free') return false;

    // Only trust explicit Free evidence. Generic upgrade/no-signal is ambiguous.
    return (
      source === 'profile' ||
      source === 'pillbox' ||
      source === 'cache-free' ||
  ...
  function isWeakAmbiguousResolution(tier, confidence, source, cachedTier) {
    if (isPaidPlanTier(cachedTier)) return false;
    if (tier) return false;
    if (confidence !== 'low') return false;
    return source === 'generic-upgrade' || source === 'no-signal';
  }
  ...
  function resolvePlanTier(signals) {
    ...
    if (fromProfile) {
      return {
        ...
        confidence: 'high',
        source: 'profile',
        signalText: normalizePlanSignalText(signals.profileText),
      };
    }
    ...
    if (fromPillbox) {
      return {
        ...
        confidence: 'medium',
        source: 'pillbox',
        signalText: normalizePlanSignalText(signals.pillboxText),
      };
    }

    if (isPaidPlanTier(signals.cachedTier)) {
      return {
        tier: signals.cachedTier,
        confidence: 'cached',
        source: 'cache-paid',
        signalText: 'Cached paid tier (' + String(signals.cachedScope || 'default') + '): ' + signals.cachedTier,
      };
    }
    ...
    if (fromModel) {
      return {
        ...
        confidence: 'medium',
        source: 'model',
        signalText: normalizePlanSignalText(signals.modelText),
      };
    }

    var fromUpsellOffer = tierFromUpsellOfferText(signals.upsellText);
    if (fromUpsellOffer) {
      return {
        tier: fromUpsellOffer,
        confidence: 'medium',
        source: 'upsell-offer',
        signalText: normalizePlanSignalText(signals.upsellText),
      };
    }
    ...
    if (hasGenericUpgrade) {
      return {
        tier: 'Unknown',
        confidence: 'low',
        source: 'generic-upgrade',
        signalText: normalizePlanSignalText(
          signals.upsellText || signals.pillboxText || 'Upgrade prompt detected'
        ),
      };
    }
    ...
    if (signals.cachedTier === 'Free') {
      return {
        ...
        confidence: 'low',
        source: 'cache-free',
        signalText: 'Cached free tier (' + String(signals.cachedScope || 'default') + '): Free',
      };
    }

    return { tier: 'Unknown', confidence: 'low', source: 'no-signal', signalText: '' };
  }
  ...
  function detectAndUpdatePlan() {
    ...
    var accountScope = syncPlanScope(window.location.href);
    var cacheKey = getScopedPlanCacheKey(accountScope);

    safeStorageGet(['manual_plan_override', cacheKey]).then(function(result) {
      ...
      var inMemoryScopedTier = (localTabTierScope === accountScope) ? localTabTier : '';
      var cachedEntry = decodeScopedTierCacheEntry(result ? result[cacheKey] : '');
      var cachedTier = normalizePlanTier(
        inMemoryScopedTier || (isScopedTierCacheUsable(cachedEntry) ? cachedEntry.tier : '')
      );
      ...
      if (!upsellText) {
        upsellText = getUpgradeOfferTextFallback();
      }
      ...
      if (overrideRaw && overrideRaw.toLowerCase() !== 'auto' && overrideTier) {
        setPlanDetectionState(
          overrideTier,
          'manual',
          'manual-override',
          'Manual override (' + accountScope + '): ' + overrideTier,
          accountScope
        );
      ...
      var resolution = resolvePlanTier({
        ...
        cachedScope: accountScope,
        cachedSource: cachedEntry.source || '',
      ...
      var resolvedSignalText = normalizePlanSignalText(resolution.signalText);
      ...
      if (isPaidPlanTier(cachedTier) && (!resolvedTier || resolvedConfidence === 'low')) {
        ...
        resolvedSignalText = 'Retained cached paid tier (' + accountScope + '): ' + cachedTier;
      }

      // Persist trusted detections so direct-chat refreshes can recover faster.
      if (!document.hidden && shouldPersistPlanTier(resolvedTier, resolvedConfidence, resolvedSource)) {
        var encodedCache = encodeScopedTierCacheEntry(resolvedTier, resolvedSource);
        if (encodedCache) {
          var cachePayload = {};
          cachePayload[cacheKey] = encodedCache;
          safeStorageSet(cachePayload);
        }
      }
      ...
      if (isKnownPlanTier(resolvedTier)) {
        if (resolvedTier !== 'Free' || shouldForceFreeFallback(resolvedTier, resolvedSource, cachedTier)) {
      ...
      if (isWeakAmbiguousResolution(resolvedTier, resolvedConfidence, resolvedSource, cachedTier)) {
        var now = Date.now();
        if (now - lastWeakPlanProbeAt >= PLAN_WEAK_REPROBE_MIN_MS) {
          lastWeakPlanProbeAt = now;
          queuePlanDetection(PLAN_WEAK_REPROBE_DELAY_MS);
        }
      }
  ...
  function resetChatState(options) {
    ...
    draftApiPromise              = null;
    draftApiPromiseSignature     = '';
    resetCanvasState('chat-change', true);
  ...
  function checkUrlChange() {
    ...
    var newCanvasRouteKey = getCanvasRouteKey();
    var canvasRouteChanged = !!(lastCanvasRouteKey && newCanvasRouteKey !== lastCanvasRouteKey);
    if (canvasRouteChanged && newId === currentChatId) {
      resetCanvasState('route-change', false);
      scheduleRecalc();
    }
    lastCanvasRouteKey = newCanvasRouteKey;
  ...
  async function recalculate() {
    ...
    var canClearHistoryAttachmentCache =
      attachSource === 'history-storage' &&
      storedHistoryAttachmentTokens > 0 &&
      trackedFileCount === 0 &&
      localAttachmentTokens <= 0 &&
    ...
    var meterBreakdown = {
      ...
      planSignal: currentPlanTierSignal,
      canClearHistoryAttachmentCache: canClearHistoryAttachmentCache,
    ...
    TokenMeterUI.update({
      ...
      tierSignal:       currentPlanTierSignal,
  ...
  function rememberStaleCanvasRoot(root) {
    if (!root || !staleCanvasRoots) return;
    try { staleCanvasRoots.add(root); } catch (_) {}
  }

  function isStaleCanvasCandidate(node) {
    if (!node || !staleCanvasRoots) return false;
    var current = node;
    var hops = 0;
    while (current && hops < 25) {
      try {
        if (staleCanvasRoots.has(current)) return true;
      } catch (_) {}
  ...
  function markVisibleCanvasRootsStale() {
    if (!staleCanvasRoots) return;
    var selectors = CANVAS_ROOT_SELECTORS.concat(CANVAS_EDITOR_SELECTORS, ['.monaco-editor']);
    for (var i = 0; i < selectors.length; i++) {
      var nodes = [];
      try { nodes = uniqueElements(deepQueryAll(selectors[i], document)); } catch (_) {}
      for (var j = 0; j < nodes.length; j++) {
        var container = null;
        try { container = findCanvasContainerFromNode(nodes[j]); } catch (_) {}
        rememberStaleCanvasRoot(container || nodes[j]);
      }
    }
  }

  function resetCanvasState(reason, markCurrentRootsStale) {
    if (markCurrentRootsStale) {
      rememberStaleCanvasRoot(canvasRoot);
      if (!canvasRoot) {
        markVisibleCanvasRootsStale();
      }
    }
    clearTimeout(canvasDebounceTimer);
    canvasDebounceTimer = null;
    canvasPendingRequestId = '';
    canvasScanSeq += 1;
    if (canvasObserver) {
    ...
    canvasMonacoLastAt = 0;
    canvasMonacoChatId = '';
    canvasMonacoRouteKey = '';
    canvasTextSource = 'none';
    debugCanvasSnapshot({
      seq: ++canvasDebugSeq,
      event: 'canvas-reset',
      reason: reason || '',
      routeKey: getCanvasRouteKey(),
      chatId: currentChatId,
    });
  }
  ...
  function requestCanvasMonacoText(root, requestChatId, requestRouteKey, scanSeq) {
    var host = root || canvasRoot;
    if (!host) return;
    var chatId = requestChatId || currentChatId || '';
    var routeKey = requestRouteKey || getCanvasRouteKey();
    if (!chatId || chatId !== currentChatId || routeKey !== getCanvasRouteKey()) return;
    if (isStaleCanvasCandidate(host)) return;
    var now = Date.now();
    ...
    var requestId = [
      chatId,
      routeKey,
      scanSeq || canvasScanSeq,
      now,
    ].join('|');
    canvasPendingRequestId = requestId;
    try {
      window.postMessage({
        type: 'GTM_CANVAS_TEXT_REQUEST',
        requestId: requestId,
        chatId: chatId,
        routeKey: routeKey,
      }, '*');
  ...
  window.addEventListener('message', function(ev) {
    try {
      ...
      if (!data.requestId || data.requestId !== canvasPendingRequestId) return;
      if (!data.chatId || data.chatId !== currentChatId) return;
      if (!data.routeKey || data.routeKey !== getCanvasRouteKey()) return;
      if (!canvasRoot || canvasRootChatId !== currentChatId || canvasRootRouteKey !== getCanvasRouteKey()) return;
      if (isStaleCanvasCandidate(canvasRoot)) return;
      canvasPendingRequestId = '';
      canvasMonacoText = data.text;
      canvasMonacoLastAt = Date.now();
      canvasMonacoChatId = currentChatId;
      canvasMonacoRouteKey = getCanvasRouteKey();
      debugCanvasSnapshot({
        seq: ++canvasDebugSeq,
        event: 'monaco-response',
        length: canvasMonacoText.length,
        chatId: currentChatId,
        routeKey: canvasMonacoRouteKey,
      });
    ...
    for (var i = 0; i < selectors.length; i++) {
      try {
        if (scope && scope.nodeType === 1 && scope.matches && scope.matches(selectors[i]) &&
            isVisibleCanvasElement(scope) && !isStaleCanvasCandidate(scope)) {
          return scope;
        }
      } catch (_) {}
      var nodes = [];
      ...
      for (var j = 0; j < nodes.length; j++) {
        if (isVisibleCanvasElement(nodes[j]) && !isStaleCanvasCandidate(nodes[j])) return nodes[j];
      }
    }
    return null;
  }
  ...
  function getCanvasEditorText(root) {
    ...
    if (canvasMonacoText && typeof canvasMonacoText === 'string' &&
        canvasMonacoChatId === currentChatId &&
        canvasMonacoRouteKey === getCanvasRouteKey() &&
        monacoAgeMs <= CANVAS_MONACO_STALE_MS) {
  ...
  function scheduleCanvasScan(root, immediate) {
    ...
    var targetRouteKey = getCanvasRouteKey();
    var targetScanSeq = ++canvasScanSeq;
    var delay = immediate ? 0 : CANVAS_DEBOUNCE_MS;
    canvasDebounceTimer = setTimeout(function() {
      if (!target) return;
      if (!targetChatId || targetChatId !== currentChatId || targetRouteKey !== getCanvasRouteKey()) return;
      if (targetScanSeq !== canvasScanSeq) return;
      if (canvasRoot && target !== canvasRoot) return;
      if (canvasRootChatId && canvasRootChatId !== currentChatId) return;
      if (canvasRootRouteKey && canvasRootRouteKey !== getCanvasRouteKey()) return;
      if (isStaleCanvasCandidate(target) || !isVisibleCanvasElement(target)) {
        if (target === canvasRoot) {
          resetCanvasState('stale-or-hidden-root', false);
          scheduleRecalc();
        }
        return;
      }
      requestCanvasMonacoText(target, targetChatId, targetRouteKey, targetScanSeq);
  ...
  function hookCanvasObserver() {
    ...
    if (!root) {
      if (canvasRoot) {
        resetCanvasState('no-visible-root', false);
        scheduleRecalc();
      }
      return;
    }
    if (isStaleCanvasCandidate(root)) return;
    if (root === canvasRoot && canvasObserver) {
      if (canvasRootChatId === currentChatId && canvasRootRouteKey === getCanvasRouteKey()) return;
      resetCanvasState('root-route-mismatch', true);
      scheduleRecalc();
      return;
    }
    if (canvasObserver) {
    ...
    canvasLastSignature = '';
    canvasRoot = root;
    canvasRootChatId = currentChatId;
    canvasRootRouteKey = getCanvasRouteKey();
    ensureCanvasBridgeInjected();
  ...
  function init() {
    ...
    lastCanvasRouteKey = getCanvasRouteKey();
    lastPlanDetectHref = window.location.href;
    syncPlanScope(lastPlanDetectHref);
